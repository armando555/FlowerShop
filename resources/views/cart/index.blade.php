            ...
            <div class="card-body">
                ...
                <form method="POST" action="{{route('cart.buy')}}">
                    ...
                    <ul>
                        ...
                        <label for="exampleInputName" class="font-weight-bold">{{__('messages.quantity')}}: {{$quantityFlower[$item->getId()]}}</label>                       
                        @endforeach
                    </ul>
                    ...
                    <ul>
                        ...
                        <label for="exampleInputName" class="font-weight-bold">{{__('messages.quantity')}}: {{$quantityBouquet[$item->getId()]}}</label>                        
                        @endforeach
                    </ul>
                    @endif
                    @endif
                    ...
                    <ul>
                        ...
                        <li>{{ $item->getId() }} - {{ $item->getName() }} : {{ $item->getPrice() }}</li>
                        <a class="btn btn-success" href="{{route('combo.show',$item->getId())}}">{{__('messages.details')}}</a>
                        <label for="exampleInputName" class="font-weight-bold">{{__('messages.quantity')}}: {{$quantityCombo[$item->getId()]}}</label>
                        @endforeach
                    </ul>
                    @endif
                    @endif
